//
//  YFNormalFinancialDetailsViewController.m
//  GoldenRiceBags
//
...
@interface YFNormalFinancialDetailsViewController ()<UIScrollViewDelegate,SectionChooseVCDelegate,UITableViewDelegate,UITableViewDataSource,YFCustomOpenTheDepositoryDelegate,YFOpenAnAccountDelegate,YFCustomPopoverDelegate>
...
@implementation YFNormalFinancialDetailsViewController
- (void)dealloc
{
...
- (void)viewDidLoad {
    ...
    [YFFinancialDetailsTableViewCell regisCellForTableView:self.tableView];
    
    
    
    self.segment = [[SectionChooseView alloc] initWithFrame:CGRectMake(0,0, WIDTH, YF_H(52)) titleArray:@[@"项目介绍",@"投资记录",@"认证及文件"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (!self.contentCell) {
        self.contentCell = [YFFinancialDetailsTableViewCell dequeueCellForTableView:tableView];
        self.contentCell.selectionStyle = UITableViewCellSelectionStyleNone;
        if (self.typeId == YFNormalFinancialDetailsViewControllerTypeMyList) {
             [self.contentCell customPageView:self.listModel.projectId];
        }else if (self.typeId == YFNormalFinancialDetailsViewControllerTypeYuanDetail){
             [self.contentCell customPageView:self.listModel.originId];
        }
        else{
              [self.contentCell customPageView:self.listModel.id];
...
-(YFNormalFinancialView *)headerView
{
    if (!_headerView) {
        YFNormalFinancialView *view = [[YFNormalFinancialView alloc] initWithFrame:CGRectMake(0, 0, WIDTH, YF_H(412))];
...
-(UIButton *)YFButton
{
    if (!_YFButton) {
        ...
        if (_type ==NO) {
           [button setTitle:@"已满标" forState:UIControlStateNormal];
            button.backgroundColor =[YFTool YFColorWithString:@"DDDDDD"];
        }
        if (_historyType ==1) {
            [button setTitle:@"已关闭" forState:UIControlStateNormal];
            button.backgroundColor =[YFTool YFColorWithString:@"DDDDDD"];
        }
        if (_historyType ==2) {
            [button setTitle:@"已结清" forState:UIControlStateNormal];
            button.backgroundColor =[YFTool YFColorWithString:@"DDDDDD"];
        }
...
-(void)DataRequest
{
    ...
    NSString *projectId = self.typeId == YFNormalFinancialDetailsViewControllerTypeList?self.listModel.id:self.listModel.projectId;
    
    switch (self.typeId) {
        case YFNormalFinancialDetailsViewControllerTypeList:
        {
            projectId = self.listModel.id;
        }
            break;
        case YFNormalFinancialDetailsViewControllerTypeMyList:
        {
            projectId = self.listModel.projectId;
        }
            break;
        case YFNormalFinancialDetailsViewControllerTypeYuanDetail:
        {
            projectId = self.listModel.originId;
        }
            break;
        default:
            break;
    }
    [YFHomePageRequest YFHomePageProjectDetail:projectId success:^(id json) {
        
        if ([YFTool Nsdic:json]==1) {
            NSLog(@"ProjectDetail == %@",json);
            NSDictionary *dic=[json objectForKey:@"data"];
            NSDictionary *loanInfoEntityDic = [dic objectForKey:@"projectEntity"];
             self.title=loanInfoEntityDic[@"projectName"];
            YFHomePageModel *model = [[YFHomePageModel alloc]init];
            [model setValuesForKeysWithDictionary:loanInfoEntityDic];
            [self.headerView setterHomePageModel:model andDic:dic];
            if ([model.residue floatValue] == 0) {
                [self.YFButton setTitle:@"已满标" forState:UIControlStateNormal];
                self.YFButton.userInteractionEnabled = NO;
...
- (void)setTypeId:(YFNormalFinancialDetailsViewControllerType)typeId
{
    _typeId = typeId;
}
