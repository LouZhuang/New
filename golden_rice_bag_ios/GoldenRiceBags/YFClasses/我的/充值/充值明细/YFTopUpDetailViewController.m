...
#import "YFTopUpDetailViewController.h"

@interface YFTopUpDetailViewController ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
/*
 tableview
 */
...
@implementation YFTopUpDetailViewController
{
    NSMutableArray *_isExpandArray;//记录section是否展开
}
static NSString *cellId = @"YFTopUpDetailTableViewCell";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    YFTopUpDetailTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (cell == nil) {
        cell = [[YFTopUpDetailTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
...
-(void)back
{
    ...
    for (UIViewController *controller in self.navigationController.viewControllers) {
            ...
            if ([controller isKindOfClass:[YFTopUpViewController class]]) {
...
-(void)DataRequest
{
    [YFMineRequest YFRechargeListString:[NSString stringWithFormat:@"%ld",_page] PageSizeString:@"10" success:^(id json) {
