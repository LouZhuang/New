...
@interface YFCloseViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation YFCloseViewController
...
-(void)DataRequest
{
    ...
    [YFMineRequest YFUidString:self.mineBorrowModel.id success:^(id json) {
        ...
        if ([YFTool Nsdic:json]==1) {
            ...
            NSString *timeStr = [YFTool timeWithTimeIntervalString:self.dataDic[@"loanDetail"][@"modifyTime"] Format:@"yyyy-MM-dd"];
            self.whyLabel.text = [NSString stringWithFormat:@"关闭时间:%@",timeStr];
            [self configuration];
