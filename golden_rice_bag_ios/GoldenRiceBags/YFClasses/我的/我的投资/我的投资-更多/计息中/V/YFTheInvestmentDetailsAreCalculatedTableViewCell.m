//
//  YFTheInvestmentDetailsAreCalculatedTableViewCell.m
//  GoldenRiceBags
//
...
@interface YFTheInvestmentDetailsAreCalculatedTableViewCell ()<UIPageViewControllerDelegate,UIPageViewControllerDataSource,UIScrollViewDelegate>
...
@implementation YFTheInvestmentDetailsAreCalculatedTableViewCell
- (void)dealloc {
    //清除监听
...
- (void)customPageView {
    ...
    YFApplicationOfFundsTableViewController *ctrl1 = [[YFApplicationOfFundsTableViewController alloc] init];
    if ([self.dataDic[@"usageInfo"] isKindOfClass:[NSDictionary class]]) {
        ctrl1.dataDic = self.dataDic[@"usageInfo"];
    }
    
    YFTheRemittancePlanTableViewController *ctrl2 = [[YFTheRemittancePlanTableViewController alloc] init];
    ctrl2.dataArr = self.dataDic[@"backPlan"];
...
+ (void)regisCellForTableView:(UITableView *)tableView {
    [tableView registerClass:[YFTheInvestmentDetailsAreCalculatedTableViewCell class] forCellReuseIdentifier:@"cellid"];
}
