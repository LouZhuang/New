//
//  TFMyRedPacketsViewController.m
//  GoldenRiceBags
//
...
@interface TFMyRedPacketsViewController ()<UIScrollViewDelegate,SectionChooseVCDelegate>
...
@implementation TFMyRedPacketsViewController
...
-(void)configuration
{
    ...
    [self SectionSelectIndex:_index];
    // 2.把对应的标题选中
    self.sectionChooseView.selectIndex = _index;
...
//    [YFProgressHUD showInfoWithStatus:@"规则说明"];
    ...
    vc.title = @"奖券规则说明";
    vc.isWord = YES;
    vc.pathUrl = [[NSBundle mainBundle]pathForResource:@"奖券规则说明" ofType:@"doc"];
...
- (void)setupUIScrollView {
    ...
    self.sectionChooseView = [[SectionChooseView alloc] initWithFrame:CGRectMake(0, YF_H(10), WIDTH, YF_H(50)) titleArray:@[@"加息券", @"红包"]];
...
-(void)setupChildViewController {
    ...
    /*
     加息券
     */
    YFRateCouponViewController *aVC = [[YFRateCouponViewController alloc] init];
    aVC.title = self.title;
    if (_selectType==1) {
        aVC.moneyString=_moneyString;
        aVC.selectType=1;
    }
    ...
    /*
     洪波啊
     */
    YFARedEnvelopeViewController *bVC = [[YFARedEnvelopeViewController alloc] init];
    bVC.title = self.title;
    if (_selectType==1) {
        bVC.moneyString=_moneyString;
        bVC.selectType=1;
    }
