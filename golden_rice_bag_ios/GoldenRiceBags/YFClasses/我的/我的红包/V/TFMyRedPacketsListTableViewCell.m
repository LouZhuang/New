//
//  TFMyRedPacketsListTableViewCell.m
//  GoldenRiceBags
//
...
@implementation TFMyRedPacketsListTableViewCell
...
-(UIView *)leftDownView
{
    if (!_leftDownView) {
        ...
        [view.layer addSublayer:[YFTool LayerInitialColor:[YFTool YFColorWithString:@"FF5D5D"] LayerEndColor:[YFTool YFColorWithString:@"FF9494"] LayerFrame:CGRectMake(0, 0, YF_W(222), YF_H(98))]];
...
-(UIView *)rightDownView
{
    if (!_rightDownView) {
        UIView *view = [[UIView alloc] init];
        view.backgroundColor=[YFTool YFColorWithString:@"FF9494"];
        view.layer.cornerRadius = YF_W(6);
        view.layer.masksToBounds = YES;
        view.userInteractionEnabled=YES;
        _rightDownView = view;
...
-(UILabel *)usingRangeLabel
{
    if (!_usingRangeLabel) {
        ...
        label.text=@"";
        label.backgroundColor=[YFTool YFColorWithString:@"FF5D5D"];
...
-(void)setterMyRedPacketsModel:(YFMyRedPacketsModel *)model
{
    ...
    self.timeLabel.text=[NSString stringWithFormat:@"剩余%@天过期",@"3"];
//    self.timeLabel.text=[NSString stringWithFormat:@"剩余%ld天过期",[YFTool getTheCountOfTwoDaysWithBeginDate:model.start endDate:model.end]];
