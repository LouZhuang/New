//
//  YFVerificationCodeViewController.m
//  GoldenRiceBags
//
...
#import "YFVerificationCodeViewController.h"
#import "GTVerifyCodeView.h"
#import "UIView+MBLayout.h"
@interface YFVerificationCodeViewController ()
...
@implementation YFVerificationCodeViewController
/*
 视图将要显示
 */
...
-(void)configuration
{
    [self.view sd_addSubviews:@[self.backButton,self.VerificationLabel,self.VerificationedLabel,self.pleaseVerificationLabel,self.downView,self.timeButton]];
    ...
    [YFLoginRequest YFPhoneString:_phoneString success:^(id json) {
        if ([YFTool Nsdic:json]==1) {
            /*
             用户存在 去登陆
             */
            YFLoginViewController *YFLoginVC=[[YFLoginViewController alloc]init];
            YFLoginVC.phoneString=_phoneString;
            [self.navigationController pushViewController:YFLoginVC animated:YES];
        ...
        else if ([YFTool Nsdic:json]==0)
        {
            /*
             用户不存在 去注册
             */
            YFReferenceCellPhoneNumberViewController *YFReferenceCellPhoneNumberVC = [[YFReferenceCellPhoneNumberViewController alloc] init];
            YFReferenceCellPhoneNumberVC.phoneString=_phoneString;
            [self.navigationController pushViewController:YFReferenceCellPhoneNumberVC animated:YES];
        }
        else{
